...
def process_honor_files():
    ...
    # 保存训练集文件路径
    with open(os.path.join(output_dir, f'train_virtual_files_split_{train_split}_{test_split}.txt'), 'w') as f:
        for path in train_files:
            f.write(f"{path}\n")

    # 保存测试集文件路径
    with open(os.path.join(output_dir, f'test_virtual_files_split_{train_split}_{test_split}.txt'), 'w') as f:
