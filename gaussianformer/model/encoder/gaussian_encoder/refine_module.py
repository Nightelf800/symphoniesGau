...
class SparseGaussian3DRefinementModule(BaseModule):
    ...
    def forward(
        ...
        if self.xyz_coordinate == 'polar':
            # 使用广播机制处理批处理维度
            rrr = xyz[..., 0] * (pc_real_range[:, 3] - pc_real_range[:, 0]).unsqueeze(-1) + pc_real_range[:,
                                                                                            0].unsqueeze(-1)
            theta = xyz[..., 1] * (pc_real_range[:, 4] - pc_real_range[:, 1]).unsqueeze(-1) + pc_real_range[:,
                                                                                              1].unsqueeze(-1)
            phi = xyz[..., 2] * (pc_real_range[:, 5] - pc_real_range[:, 2]).unsqueeze(-1) + pc_real_range[:,
                                                                                            2].unsqueeze(-1)
            ...
            # 笛卡尔坐标系下的批处理计算
            xxx = xyz[..., 0] * (pc_real_range[:, 3] - pc_real_range[:, 0]).unsqueeze(-1) + pc_real_range[:,
                                                                                            0].unsqueeze(-1)
            yyy = xyz[..., 1] * (pc_real_range[:, 4] - pc_real_range[:, 1]).unsqueeze(-1) + pc_real_range[:,
                                                                                            1].unsqueeze(-1)
            zzz = xyz[..., 2] * (pc_real_range[:, 5] - pc_real_range[:, 2]).unsqueeze(-1) + pc_real_range[:,
                                                                                            2].unsqueeze(-1)
