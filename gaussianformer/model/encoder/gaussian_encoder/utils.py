...
def cartesian(anchor, pc_range):
    xyz = safe_sigmoid(anchor[..., :3])
    xxx = xyz[..., 0] * (pc_range[:, 3] - pc_range[:, 0]).unsqueeze(-1) + pc_range[:, 0].unsqueeze(-1)
    yyy = xyz[..., 1] * (pc_range[:, 4] - pc_range[:, 1]).unsqueeze(-1) + pc_range[:, 1].unsqueeze(-1)
    zzz = xyz[..., 2] * (pc_range[:, 5] - pc_range[:, 2]).unsqueeze(-1) + pc_range[:, 2].unsqueeze(-1)
    xyz = torch.stack([xxx, yyy, zzz], dim=-1)
