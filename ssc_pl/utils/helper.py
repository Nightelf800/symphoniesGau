...
def vox2pix(cam_E, cam_K, vol_origin, vox_size, img_shape, scene_size, pc_range, filepath):
    """
    ...
    # print(f'vol_bnds.shape: {vol_bnds.shape}')
    # print(f'vol_bnds: {vol_bnds}')

    # 定义一个很小的误差范围
    epsilon = 1e-8

    # 计算体素的维度，减去误差范围以避免浮点数精度问题
    vol_dim = np.ceil(((vol_bnds[:, 1] - vol_bnds[:, 0]) / vox_size) - epsilon).copy(order='C').astype(int)
    # print(f'vol_dim.shape: {vol_dim.shape}')
    ...
    # print(f'vox_coords.shape: {vox_coords.shape}')

    # Project voxels' centroid from lidar coordinates to camera coordinates
    pc_range_offsets = np.array(pc_range[:3], dtype=np.float32)
    cam_pts = fusion.TSDFVolume.vox2world(pc_range_offsets, vol_origin, vox_coords, vox_size)
    # print(f'-------world---------')
    # print(f'cam_pts_zero_element: {np.any(cam_pts[:, 2] == 0)}')
    # print(f'cam_pts_world.shape: {cam_pts.shape}')
    # if np.any(cam_pts == 0):
    ...
    # print(f'-------camera---------')
    # print(f'cam_pts_zero_element: {np.any(cam_pts[:, 2] == 0)}')
    # print(f'cam_pts_camera.shape: {cam_pts.shape}')
    # if np.any(cam_pts == 0):
    #     print(f'filepath: {filepath}')
...
def get_meshgrid(ranges, reso):
    nx = int((ranges[3] - ranges[0]) / reso)
    ny = int((ranges[4] - ranges[1]) / reso)
    nz = int((ranges[5] - ranges[2]) / reso)

    xxx = np.arange(nx, dtype=float) * reso + ranges[0]
    yyy = np.arange(ny, dtype=float) * reso + ranges[1]
    zzz = np.arange(nz, dtype=float) * reso + ranges[2]

    # 使用 np.newaxis 增加维度
    xxx = xxx[:, np.newaxis, np.newaxis]  # shape: (grid[0], 1, 1)
    yyy = yyy[np.newaxis, :, np.newaxis]  # shape: (1, grid[1], 1)
    zzz = zzz[np.newaxis, np.newaxis, :]  # shape: (1, 1, grid[2])

    # 使用 np.tile 扩展数组
    xxx = np.tile(xxx, (1, ny, nz))  # shape: (grid[0], grid[1], grid[2])
    yyy = np.tile(yyy, (nx, 1, nz))  # shape: (grid[0], grid[1], grid[2])
    zzz = np.tile(zzz, (nx, ny, 1))  # shape: (grid[0], grid[1], grid[2])
