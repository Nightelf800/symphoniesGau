...
class TSDFVolume:
    ...
    @staticmethod
    @njit(parallel=False)
    def vox2world(pc_range, vol_origin, vox_coords, vox_size, offsets=(0.5, 0.5, 0.5)):
        ...
        # cam_pts = pc_range + vol_origin + vox_size * vox_coords + vox_size * offsets

        cam_pts = np.empty_like(vox_coords, dtype=np.float32)
        for i in prange(vox_coords.shape[0]):
            for j in range(3):
                cam_pts[i, j] = vol_origin[j] + vox_size * vox_coords[i, j] + vox_size * offsets[j] + pc_range[j]
        return cam_pts
    ...
    def cam2pix(cam_pts, intr):
        ...
        # # numpy算子改进
        # # 防止除以零
        # cam_pts[:, 2] = np.where(cam_pts[:, 2] == 0, 1e-8, cam_pts[:, 2])
        
        # # 使用 NumPy 的广播机制进行向量化计算
        # pix_x = np.round((cam_pts[:, 0] * fx / cam_pts[:, 2]) + cx).astype(np.int64)
        # pix_y = np.round((cam_pts[:, 1] * fy / cam_pts[:, 2]) + cy).astype(np.int64)
        
        # # 将计算结果组合成最终的像素坐标数组
        # pix_x_expanded = np.expand_dims(pix_x, axis=1)  # 将 pix_x 扩展一个维度
        # pix_y_expanded = np.expand_dims(pix_y, axis=1)  # 将 pix_y 扩展一个维度
        # pix = np.concatenate((pix_x_expanded, pix_y_expanded), axis=1)  # 沿着新的轴连接
        return pix
    ...
    def integrate(self, color_im, depth_im, cam_intr, cam_pose, pc_range, obs_weight=1.):
        ...
        if self.gpu_mode:  # GPU mode: integrate voxel volume (calls CUDA kernel)
            ...
            cam_pts = self.vox2world(pc_range, self._vol_origin, self.vox_coords, self._voxel_size)
